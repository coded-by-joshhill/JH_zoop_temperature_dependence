...
library(performance) # for model assessment
...
group_order <- c("Crustaceans", "GelPreds", "GelFilter")


# Combine them into one dataframe
usedat <- rbind(cleardat, ingdat, grwdat, respdat) %>% 
  filter_out(funcGrp == "OTHER" | is.na(temp_C)) %>%  # filter out the func group "OTHER" and remove any NAs in temp_C
...
m2 <- glmmTMB(ln_Cspecific_rate ~ 
  ...
  # temp:grp - looks like there are significantly diff temp dep for GelPreds to Cust but GelFilters are not diff to GelPreds???
  # rate:grp - appears to be signif diff among rates and funcGrp but this doesn't include temperature in the interaction...
...
performance::compare_models(m1, m2, m3)
