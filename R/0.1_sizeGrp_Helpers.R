# Helper functions for temperature sensitivity data analysis for sizeGrps
# Josh Hill
# 09/02/2026
...
boot_Q10 <- function(df) {
  ...
  out <- glmmTMB(y ~ x * sizeGrp + (1|primRef) + (1|taxa),  data = d) # fit the model
...
get_results <- function(df, groups, coefs_list, Q10pdat) {
  ...
  results <- map(groups, function(g) {
    ...
    x_vals <- df %>% filter(sizeGrp == g) %>% pull(x)
    ...
    preds <- imap_dfr(coefs_list, function(fx, i) {
      ...
      intercept <- fx["(Intercept)"] + ifelse(paste0("sizeGrp", g) %in% names(fx), fx[paste0("sizeGrp", g)], 0)
      
      # Calculate group-specific slope (base slope + group interaction effect)
      slope <- fx["x"] + ifelse(paste0("x:sizeGrp", g) %in% names(fx), fx[paste0("x:sizeGrp", g)], 0)
...
arrhenius_plot <- function(mdat, rate_col, results, 
  ...
  plot_data <- map(names(results), function(g) {
    ...
    list(
      data = df_data %>% mutate(sizeGrp = g),
      pred = df_pred %>% mutate(sizeGrp = g),
      Q10_label = paste0(
  ...
  all_labels <- tibble(
    sizeGrp = map_chr(plot_data, \(x) as.character(unique(x$data$sizeGrp))),
    label = map_chr(plot_data, \(x) x$Q10_label),
    x = 0.0037,
    y = Inf
  )
  
  # Re-order sizeGrps if specified
  if (!is.null(group_order)) {
    all_data <- all_data %>%
      mutate(sizeGrp = fct_relevel(sizeGrp, group_order))
  ...
  p <- ggplot() +
    ...
    facet_wrap(~ sizeGrp, scales = "free_y", ncol = ncol) +
