# Calculating allZoop growth rate Q10s
# Josh Hill
# 10/02/2026
...
m1 <- glmmTMB(y ~ x + (1|primRef) + (1|taxa),  data = mdat) # with primRef and taxa as random intercepts
  ...
  plot(sim)
  # QQplot doesn't look great. Let's see if we can improve the fit by adding a random slope to taxa

m2 <- glmmTMB(y ~ x + (1|primRef) + (x|taxa),  data = mdat) # with primRef as random intercepts and taxa as random intercept and slope

  # Check diagnostics
  sim <- simulateResiduals(m2)
  plot(sim)
  # QQplot looks slightly better and residuals are a little more scattered

  # Look at anova results for m1 and m2  
  anova(m1, m2)
    # m2 is slightly better AIC and because the diagnostics were better, I'll continue with m2
  summary(m2)
  r.squaredGLMM(m2)
...
results <-  analyse_thermal_sensitivity(m2, mdat)
...
# Save it
# saveRDS(Q10dat, "Data/allZoop_analysis/allZoop_growth_Q10_data.rds")
...
# saveRDS(arrhenius_obj, "Data/allZoop_analysis/allZoop_growth_arrhenius_data.rds")
