...
# Read in the data ----
# First, estimate some growth rates based on reported mass

# Goldstein2023
GoldsteinDat <- read_csv("Data/rawData/GrowthEstimates/Goldstein2023_growth_estimates.csv",
                         skip = 1) %>% 
...
# Luskow2016
LuskowDat <- read_csv("Data/rawData/GrowthEstimates/Luskow2016_growth_estimates.csv",
                      skip = 1) %>% 
...
# Rey2001
ReyDat <- read_csv("Data/rawData/GrowthEstimates/Rey2001_growth_estimates.csv",
                   skip = 1) %>% 
...
dat <- read_csv("Data/rawData/Grwth_dat.csv") %>%
...
datFinal <- datClean %>%
  ...
  select(-c(BM_wet, BM_dry, BM_C, weight_unit, weight_calc)) %>%
