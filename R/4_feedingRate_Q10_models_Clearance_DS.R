...
results <- mdat %>%
  group_by(zoopGrp) %>%
  group_split() %>%
  map(~analyse_thermal_sensitivity(m1, .x)) #*** Using purrr is soooo much better
...
# Trying with bootstrapping the confidence intervals
  boot_Q10 <- function(df) {
    ...
    out <- glmmTMB(y ~ x * zoopGrp + (1|primRef) + (1|taxa),  data = d)
    }

  boot_models <- map(1:999, ~boot_Q10(mdat)) # 999 bootstrap models...you could do 9999 to be sure...it'll just be a little slow although you could check furrr or even the new version of purrr for how to run in parallel
  ...
  get_Q10s <- function(m) {
    ...
    Q10_by_group <- function(d) {
      ...
      T2 <- T1 + 10 # Take minimum temp and add specified temperature (10 by default) #***HARDWIRED TO 10, here
  ...
  Q10_estimates <- map_dfr(boot_models, get_Q10s) # Again, would be faster in parallel
  ...
  Q10_estimates %>% 
    ...
    ggplot() +
      ...
      geom_point(aes(x = Group, y = median),
                 size = 3) +
    labs(y = "Q10", x = "Zoop Group")
  # MUCH more stable confidence intervals
