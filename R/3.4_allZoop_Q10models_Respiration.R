# Calculating sizeGrp respiration rate Q10s and plotting
# Josh Hill
# 09/02/2026



  # Here I read in clean respiration rate data
...
library(tidyverse)
library(parallel)
source("R/0_sizeGrp_Helpers.R")



# Read in the data ----
dat <- readRDS("Data/resp_dat.rds") %>% 
  filter(sizeGrp != "OTHER") # remove any unclassified zooplankton
...
mdat <- usedat %>% 
    filter( # exclude values that are not biologically reasonable or are extreme outliers
      (sizeGrp == "Mesoplankton"    & Cspecific_rate < 60) |
      (sizeGrp == "Macroplankton"   & Cspecific_rate < 60)) %>%
    filter(!is.na(temp_C)) %>% 
...
# Using glmmTMB fit (y ~ x * sizeGrp + (1|primRef) + (1|taxa))...this takes some time...
...
# Get Q10 estimates from bootstrap models...this takes some time too...
...
# Save as RDS
# saveRDS(Q10_estimates, "Data/sizeGrp_Q10_estimates_respiration.rds") # estimates
# saveRDS(mdat, "Data/sizeGrp_respiration_mdat.rds") # save modelling dataframe for plotting
# saveRDS(Q10pdat, "Data/sizeGrp_Q10_summary_respiration.rds") # median and confidence intervals
...
# saveRDS(arrhenius_obj, "Data/sizeGrp_respiration_arrhenius_plot_data.rds")
